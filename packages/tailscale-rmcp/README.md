...
`npx -y @dinglebear/rtailscale-mcp devices --json` -> start loopback HTTP with
`TAILSCALE_MCP_HOST=127.0.0.1 npx -y @dinglebear/rtailscale-mcp serve` -> call `tools/call`
with `{"action":"devices"}`.
...
| npm package | `@dinglebear/rtailscale-mcp` |
...
|---|---|---|---|
| npm / npx | `npx -y @dinglebear/rtailscale-mcp --help` | Local MCP clients and quick trials. | Downloads the matching `rtailscale` binary from GitHub Releases. |
...
```bash
npx -y @dinglebear/rtailscale-mcp --help
...
|---|---:|---|---|
| MCP stdio | Supported | `rtailscale mcp`, `npx -y @dinglebear/rtailscale-mcp mcp` | Local child-process MCP clients. |
...
- The npm package name is `@dinglebear/rtailscale-mcp`; binary aliases are
...
- `plugins/tailscale/.mcp.json` must launch `npx -y @dinglebear/rtailscale-mcp mcp` so
